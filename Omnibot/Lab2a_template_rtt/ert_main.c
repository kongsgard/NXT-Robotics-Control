...
void ecrobot_device_initialize(void)
{
  ...
  Lab2a_template_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(Lab2a_template_M));
  rtExtModeCheckInit(2);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(Lab2a_template_M->extModeInfo, 2, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(Lab2a_template_M, true);
    }
  }
...
TASK(OSEK_Task_main)
{
  ...
  display_string("-----------------");
  display_goto_xy(0, 6);
  display_string("Lab2a_template");
  display_goto_xy(0, 7);
  display_string("-----------------");
  while (1) {
    WaitEvent(TIMESUP);
    ClearEvent(TIMESUP);
    Lab2a_template_output();
...
TASK(OSEK_Task_background)
{
  int status;
  int link_status = 0;
  while ((rtmGetErrorStatus(Lab2a_template_M) == (NULL)) && !rtmGetStopRequested
         (Lab2a_template_M)) {
    ...
    if (timesup == 0) {
      ...
      {
        boolean_T rtmStopReq = false;
        rtExtModePauseIfNeeded(Lab2a_template_M->extModeInfo, 2, &rtmStopReq);
        if (rtmStopReq) {
          rtmSetStopRequested(Lab2a_template_M, true);
        }

        if (rtmGetStopRequested(Lab2a_template_M) == true) {
      ...
      {
        boolean_T rtmStopReq = false;
        rtExtModeOneStep(Lab2a_template_M->extModeInfo, 2, &rtmStopReq);
        if (rtmStopReq) {
          rtmSetStopRequested(Lab2a_template_M, true);
        }
      }
